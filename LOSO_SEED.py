...
data_path = "D:/Code_reproduction/SNN-LSTM/Data/SEED/Extracted_Features"
time_steps = 235
num_classes = 3
batch_size = 32
total_epochs = 150
...
class AdaptiveMembraneLoss(nn.Module):
    def __init__(self, alpha_base=0.15, beta_base=0.3, decay_rate=0.98,
...
class MLP(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, drop_rate=0.5):
        super().__init__()
...
class SpikeEventConsistencyFusion(nn.Module):
    def __init__(self, dim, tau=2.0, drop_rate=0.5):
        super().__init__()
        self.dim = dim
...
class SpatioTemporalSSA(nn.Module):
    def __init__(self, dim, num_heads=4, drop_rate=0.5):
        super().__init__()
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=2., drop_rate=0.5):
        super().__init__()
...
class SpikeTransformer(nn.Module):
    def __init__(self, seq_len, in_features, embed_dim, num_heads, num_layers, num_classes, T=5):
        super().__init__()
        self.T = T
...
class PureSpatioTemporalSNN(nn.Module):
    ...
    def forward(self, x, epoch=None):
        ...
        spatial_feats = spatial_feats.to(x.device)
...
def load_seed_features(path):
    ...
    label_data = sio.loadmat(os.path.join(path, 'label.mat'))
    label_mapping = {-1: 0, 0: 1, 1: 2}
    true_labels = np.vectorize(label_mapping.get)(label_data['label'][0])
    mat_files = sorted([f for f in os.listdir(path) if f.endswith('.mat') and 'label' not in f.lower()])

    for file in mat_files:
        data = sio.loadmat(os.path.join(path, file))
        subject_id = int(re.findall(r'\d+', file)[0])
        ...
        for i in range(15):
            ...
            if T < 235:
                feat = np.vstack([feat, np.zeros((235 - T, 310))])
            else:
                feat = feat[:235]
...
def split_loso_cv(X, y, sids, test_subject_id):
    train_mask = sids != test_subject_id
    test_mask = sids == test_subject_id
...
def compute_spike_sparsity(x):
    return (x > 0).sum().item() / x.numel()
...
def train_loso_fold(fold_idx, test_subject_id, X_train, y_train, X_test, y_test,
                    device, total_epochs=150, seed=42):
    print(f"\nFold {fold_idx + 1}: 测试被试 {test_subject_id} | 训练集: {len(X_train)} | 测试集: {len(X_test)}")

    checkpoint_dir = "checkpoints_spatiotemporal_snn"
    ...
    grad_clipper = AdaptiveGradientClipper(clip_value=1.0, mode='adaptive')

    modulation_monitor = ModulationMonitor()
    ...
    for epoch in range(1, total_epochs + 1):
        ...
        modulation_monitor.reset()

        for xb, yb in train_loader:
            functional.reset_net(model)
            xb = augment_eeg_data(xb, noise_level=0.02)

            pred, layered_membranes, spk, modulations = model(xb, epoch=epoch)
            modulation_monitor.update(modulations)
        ...
        model.eval()
        functional.reset_net(model)
        test_total, test_correct, test_running_loss = 0, 0, 0.0
        epoch_test_preds, epoch_test_targets = [], []
        ...
        with torch.no_grad():
            for xb, yb in test_loader:
                ...
                total_loss, _, _, _ = loss_fn(pred, yb, layered_membranes)
                test_running_loss += total_loss.item() * yb.size(0)
                test_total += yb.size(0)
                test_correct += (pred.argmax(1) == yb).int().sum().item()

                epoch_test_preds.extend(pred.argmax(1).cpu().numpy())
                epoch_test_targets.extend(yb.cpu().numpy())
        ...
        if test_acc > best_test_acc:
            ...
            best_test_preds = epoch_test_preds.copy()
            best_test_targets = epoch_test_targets.copy()

            checkpoint_path = os.path.join(checkpoint_dir, f"best_model_fold_{fold_idx + 1}_subj_{test_subject_id}.pth")
            torch.save({
                'fold': fold_idx + 1,
        ...
        if epoch % 50 == 0 or epoch == total_epochs:
            print(f"  Epoch {epoch:03d}/{total_epochs} | Train Acc: {correct / total:.4f} | Test Acc: {test_acc:.4f} | Best Acc: {best_test_acc:.4f}")
    ...
    with torch.no_grad():
        for xb, yb in test_loader:
            ...
            total_loss, _, _, _ = loss_fn(out, yb, layered_membranes)
            test_loss += total_loss.item() * yb.size(0)
            test_total += yb.size(0)
            final_test_preds.extend(out.argmax(1).cpu().numpy())
            final_test_targets.extend(yb.cpu().numpy())

    final_test_loss = test_loss / test_total
    final_test_acc = np.mean(np.array(final_test_preds) == np.array(final_test_targets))

    print(f"--> Fold {fold_idx + 1} 完成 | 最终测试准确率: {final_test_acc:.4f} | 最佳准确率: {best_test_acc:.4f} (Epoch {best_test_epoch})")
    ...
    return {
        ...
        'best_test_targets': best_test_targets,
...
def save_detailed_results(all_fold_results, avg_final_acc, std_final_acc, avg_best_acc, std_best_acc):
    os.makedirs("results_spatiotemporal_snn", exist_ok=True)
    with open("results_spatiotemporal_snn/loso_cv_summary.txt", "w") as f:
        f.write("留一被试交叉验证（LOSO-CV）实验结果汇总\n")
        f.write("=" * 60 + "\n\n")
        for result in all_fold_results:
            f.write(f"被试 {result['test_subject_id']}: 最佳准确率 = {result['best_test_acc']:.4f} (epoch {result['best_test_epoch']}), 最终准确率 = {result['final_test_acc']:.4f}\n")
        f.write("-" * 60 + "\n")
        f.write(f"平均最终准确率: {avg_final_acc:.4f} ± {std_final_acc:.4f}\n")
        f.write(f"平均最佳准确率: {avg_best_acc:.4f} ± {std_best_acc:.4f}\n")


def main():
    print("开始留一被试交叉验证（LOSO-CV）实验...")
    global_seed = 1024
    ...
    features, labels, subject_ids = load_seed_features(data_path)
    unique_subject_ids = np.unique(subject_ids)
    print(f"加载成功，共 {len(unique_subject_ids)} 个被试。")

    all_fold_results = []
    os.makedirs("results_spatiotemporal_snn", exist_ok=True)

    for fold_idx, test_subject_id in enumerate(unique_subject_ids):
        X_train, y_train, X_test, y_test = split_loso_cv(features, labels, subject_ids, test_subject_id)
        fold_result = train_loso_fold(
            fold_idx=fold_idx,
            test_subject_id=test_subject_id,
            X_train=X_train,
            y_train=y_train,
    ...
    final_accs = [r['final_test_acc'] for r in all_fold_results]
    best_accs = [r['best_test_acc'] for r in all_fold_results]
    avg_final_acc = np.mean(final_accs)
    std_final_acc = np.std(final_accs)
    avg_best_acc = np.mean(best_accs)
    std_best_acc = np.std(best_accs)

    print("\n" + "=" * 50)
    print(f"实验完成！平均最佳准确率: {avg_best_acc:.4f} ± {std_best_acc:.4f}")
    print(f"平均最终准确率: {avg_final_acc:.4f} ± {std_final_acc:.4f}")
    print("=" * 50)

    save_detailed_results(all_fold_results, avg_final_acc, std_final_acc, avg_best_acc, std_best_acc)
